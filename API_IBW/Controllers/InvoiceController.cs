...
namespace API_IBW.Controllers
{
    ...
    public class InvoiceController : ApiController
    {
        ...
        //author:Vamsi 
        //date: 26/03/2020
        //To get all quote related invoice details by invoice id
        [Route("get-quote-related-invoice-details-by-invoice-id")]
        [HttpGet]
        public IHttpActionResult getQuoterelatedInvoiceDetailsbyInvoiceId(long quoteId, long invoiceId)
        {
            Response<InvSitescontactsQuoteDetails> output = new Response<InvSitescontactsQuoteDetails>();
            InvSitescontactsQuoteDetails InvSitesListcontactsQuoteDetails = new InvSitescontactsQuoteDetails();
            List<ClientContacts> contacts = new List<ClientContacts>();
            ...
            try
            {
                ...
                List<InvSites> data = invoiceMethods.GetQuoteInvoicesDetailsForEdit(quoteId, null, invoiceId);
                    quotedetails = invoiceMethods.GetQuoteDetailsbyId(quoteId, invoiceId);
                    if (quotedetails != null && quotedetails.int_client_id != null)
                        contacts = invoiceMethods.GetContactDetailsbyClientId(quotedetails.int_client_id);
                if (data.Count > 0 || quotedetails != null || contacts != null)
                {
        ...
        public IHttpActionResult UpsertInvoiceandInvoiceItems([FromBody]UpsertInvoiceParameters parameters)
        {
            ...
            try
            {
                ...
                {
                    string invoiceuniquenesscheckmessage = invoiceMethods.CheckInvoiceNumberUniquness(parameters.quotedetails.int_invoice_id, parameters.quotedetails.int_quote_id, parameters.quotedetails.auto_invoice_number, parameters.quotedetails.manual_invoice_number);
                            ...
                            foreach (var sow in site.sows)
                            {
                                if (sow.feestructure == "Fixed Rate" && sow.tasks[0].actions[0].isselect == true)
                                {
                                    if (sow.tasks[0].actions[0].percentinvoiced > 0)
                                    {
                                        ...
                                        InvoiceItem.int_invoice_item_id = sow.tasks[0].actions[0].int_invoice_item_id;
                                        ...
                                        InvoiceItem.dec_invoiced_percent = sow.tasks[0].actions[0].int_invoice_item_id==null ? sow.tasks[0].actions[0].percentinvoiced: (((sow.tasks[0].actions[0].invoiced+ sow.tasks[0].actions[0].billed)/ sow.quotedAmount)*100)==0?100: ((sow.tasks[0].actions[0].invoiced + sow.tasks[0].actions[0].billed) / sow.quotedAmount) * 100;
                                        InvoiceItem.dec_invoiced_total = sow.tasks[0].actions[0].int_invoice_item_id == null? sow.tasks[0].actions[0].invoiced: sow.tasks[0].actions[0].invoiced + sow.tasks[0].actions[0].billed;
                                    ...
                                    foreach (var task in sow.tasks)
                                    {
                                        foreach (var action in task.actions.Where(x => x.isselect == true))
                                        {
                                            if (action.percentinvoiced > 0)
                                            {
                                                isactionsavailable++;
                                                InvoiceItems InvoiceItem = new InvoiceItems();
                                                InvoiceItem.int_invoice_item_id = action.int_invoice_item_id;
                                                ...
                                                InvoiceItem.dec_invoiced_percent = action.int_invoice_item_id==null ? action.percentinvoiced:(((action.invoiced+action.billed)/(action.total+action.worked))*100)==0?100: ((action.invoiced + action.billed) / (action.total + action.worked)) * 100;
                                                InvoiceItem.dec_invoiced_total = action.invoiced + action.billed;
                                                InvoiceItems.Add(InvoiceItem);
                                            }
                                            else
                                            {
                    ...
                    breakloops:
                        if (isinvoicepercentvalid && isactionsavailable > 0)
                        {
                            ...
                            if (result > 0)
                            {
                                GenerateInvoicePDF(parameters.quotedetails.auto_invoice_number, parameters.quotedetails.dt_invoice_date!= null?parameters.quotedetails.dt_invoice_date.Value.ToString("yyyy-MM-dd"):"", result, headerUserId);
                                output.Status = true;
                        ...
                        {
                            output.Status = false;
                            output.Message = "Please select at least 1 item to proceed further";
                        }
                    }
                    else
                    {
        ...
        [Route("send-invoice-details")]
        [HttpPost]
        public IHttpActionResult SendInvoiceDetails()
        {
            Response<InvoicepaymentDetails> output = new Response<InvoicepaymentDetails>();
            try
            {
                ...
                var httprequest = HttpContext.Current.Request;
                //var data = JObject.Parse(httprequest.Form["InvoicePaymentDetailsObject"]).ToObject<InvoicepaymentDetails>();
                InvoicepaymentDetails data = JsonConvert.DeserializeObject<InvoicepaymentDetails>(httprequest.Form["SendInvoiceDetailsObject"]);
                //var s = JsonExtensions.FromDelimitedJson<InvoicepaymentDetails>(new StringReader(httprequest.Form["InvoicePaymentDetailsObject"])).ToList();
                if (!string.IsNullOrEmpty(data.FromEmail) && !string.IsNullOrEmpty(data.ToEmail) && !string.IsNullOrEmpty(data.subject) && !string.IsNullOrEmpty(data.email))
                {
                    if (data != null)
                    {
                        data.UserId = headerUserId;
                        long validemailcount = 0;
                        List<string> ToEmails = data.ToEmail.Split(',').Distinct().ToList();
                        foreach(var toemail in ToEmails)
                        {
                            bool isEmail = Regex.IsMatch(toemail, @"\A(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\Z", RegexOptions.IgnoreCase);
                            if (isEmail)
                            {
                                validemailcount++;
                            }
                        }
                        if (validemailcount== ToEmails.Count())
                        {
                            byte[] invoicepdf = GenerateInvoicePDFinByteArray(data.vc_invoice_number, data.vc_invoice_date, data.InvoiceId, data.UserId);
                            string invoicepdfstring = Convert.ToBase64String(invoicepdf);
                            Mails mail = new Mails();
                            List<Email> EmailCCs = new List<Email>();
                            if (data.vc_project_manager_email != null)
                            {
                                if (Regex.IsMatch(data.vc_project_manager_email, @"\A(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\Z", RegexOptions.IgnoreCase))
                                    EmailCCs.Add(new Email(data.vc_project_manager_email));
                            }
                            if (data.vc_financial_email != null)
                            {
                                if (Regex.IsMatch(data.vc_financial_email, @"\A(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?)\Z", RegexOptions.IgnoreCase))
                                    EmailCCs.Add(new Email(data.vc_financial_email));
                            }
                            //EmailCCs.Add(new Email("vamsi@optionmatrix.com"));
                            //EmailCCs.Add(new Email("k.vamshi16@yahoo.com"));
                            //data.ToEmail = "k.vamshi16@gmail.com";
                            var UserDetails = adminMethods.GetAllIBWUsers().Where(x => x.int_user_id == data.UserId).FirstOrDefault();
                            string invoicefilename = UserDetails != null ? data.vc_invoice_number + "-" + UserDetails.vc_user_name + " " + UserDetails.vc_alias_name + "-" + data.vc_invoice_date : data.vc_invoice_number + "-" + data.vc_invoice_date;
                            invoiceMethods.UpdateSentInvoice(data.InvoiceId, data.vc_sent_to, data.vc_sent_by, data.UserId);
                            bool result = mail.SendInvoice(data.subject, data.email, data.description, ToEmails.Distinct().ToList(), invoicepdfstring, data.FromEmail, invoicefilename, EmailCCs);
                            if (result)
                            {
                                output.Message = "Invoice sent successfully";
                                output.Status = true;
                            }
                            else
                            {
                        ...
                        {
                            output.Message = "Please enter valid To email(s)";
                            output.Status = false;
                        }
                    }
                    else
                ...
                {
                    output.Message = !string.IsNullOrEmpty(data.FromEmail) ? !string.IsNullOrEmpty(data.ToEmail) ? !string.IsNullOrEmpty(data.subject) ? "Please enter email body" : "Please enter subject" : "Please enter to email" : "Please enter from email";
                    output.Status = false;
                }
                return Ok(JObject.FromObject(output));
            }
